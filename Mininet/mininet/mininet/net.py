...
class Mininet(object):
    ...
    def iperfall(self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
                 seconds=15, port=5001):
        all_outputs = []
        if not hosts:
            hosts = self.hosts
            output('*** Ping: testing ping reachability\n')
        servport = port
        for server in hosts:
            server.cmdPrint('iperf -p ' + str(servport) + ' -s &')
            servport = servport + 1
        sleep(2)
        for server in hosts:
            output('%s -> ' % server.name)
            for client in hosts:
                if client != server:
                    client.cmdPrint('iperf -p ' + str(port) + ' -t ' + str(seconds) + ' -c' + server.IP() + ' &')
            port = port + 1
        return all_outputs
